...
while($row = mysqli_fetch_array($result)) { 
  ...
  echo  $row["status"];;
   echo "<br>";
  if($st==1){
      echo "<a href='member/index.php?id=$row[auction_id]&act=pay&p=$row[product_price_bid]&do=payform' class='btn btn-warning btn-xs' target='_blank'>ชำระเงิน</a>";
  }else{

     echo "<a href='member/index.php?auction_id=$row[auction_id]&act=showslip&p=$row[product_price_bid]&do=payform' class='btn btn-success btn-xs' target='_blank'>การชำระเงิน</a>";
